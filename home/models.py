...
class HomePage(Page):
    ...
    def get_context(self, request):
        ...
        if len(blogs_current_user) is not 0:
            ...
            last_blog = blogs_current_user[last_blog_index].last_published_at.replace(tzinfo=None) # removed info to make both dates Naive
            delta = today - last_blog

        if len(blogs_current_user) == 0:
            context['welcome_msg'] += 'Time to go surfing and registering it !<br><br>'
        elif delta.days > 5:
            context['welcome_msg'] += 'SORT IT OUT !!! <br><br>'
